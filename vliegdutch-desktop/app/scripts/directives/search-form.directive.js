angular.module('vliegdutch').directive('searchForm', ['$state', '$timeout', '$stateParams', '$filter', 'Flight', 'Location',
    function ($state, $timeout, $stateParams, $filter, Flight, Location) {
        ...
        return {
            ...
            link: function ($scope) {
                ...
                $scope.passengersSummary = function () {
                    ...
                    if ($scope.babies > 0) {
                        result += ', ' + $scope.babies + ' ' + ($scope.babies > 1
